...
class DownloaderPatcher(private val logger: BDLogger) {
    ...
    fun patchUrl(newUrl: String): ByteArray {
        ...
        logger.info(" -> Patching downloader with URL: '$newUrl'")
        ...
        for (method in classNode.methods) {
            for (insn in method.instructions) {
                if (insn is LdcInsnNode && insn.cst == DOWNLOADER_URL_PLACEHOLDER) {
                    insn.cst = newUrl
                    patched = true
                    logger.debug("  [ASM-PATCH] Replaced downloader placeholder in ${classNode.name}")
                    break
                }
            }
            if (patched) break
