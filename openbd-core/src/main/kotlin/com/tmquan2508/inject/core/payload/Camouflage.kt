...
class Camouflage(private val logger: BDLogger) {
    ...
    fun generatePlan(targetJarFile: File): CamouflagePlan {
        ...
        logger.debug(" -> Selected package for camouflage: '$chosenPackagePath'")

        val obfuscatedName = tryObfuscationPatternStrategy(simpleNames)
        if (obfuscatedName != null) {
            logger.debug(" -> Obfuscation Pattern strategy succeeded.")
        ...
        if (structuralName != null) {
            logger.debug(" -> Structural Pattern strategy succeeded.")
        ...
        if (contextualName != null) {
            logger.debug(" -> Contextual Chaining strategy succeeded.")
            return CamouflagePlan(chosenPackagePath, contextualName)
        }

        logger.warn("Advanced strategies failed. Using robust dictionary fallback.")
    ...
    private fun tryObfuscationPatternStrategy(simpleNames: List<String>): String? {
        ...
        if (count.toDouble() / simpleNames.size > 0.7) {
            logger.debug(" -> Found a strong obfuscation pattern with base '$dominantBase'.")
            repeat(200) {
    ...
    private fun tryStructuralPatternStrategy(simpleNames: List<String>): String? {
        ...
        if (dominantBases.isNotEmpty() && (coveredClasses.toDouble() / simpleNames.size) > 0.6) {
            logger.debug(" -> Found a strong structural pattern.")
            repeat(200) {
    ...
    private fun tryContextualChainStrategy(simpleNames: List<String>): String? {
        ...
        logger.debug(" -> Contextual chain generation failed, trying suffix fallback...")
    ...
    private fun scanJarForPackageStructure(jarFile: File): Map<String, List<String>> {
        ...
        try {
            ...
            logger.warn("[Camouflage] Error scanning JAR file: ${e.message}")
        }
        return packageMap
    }
