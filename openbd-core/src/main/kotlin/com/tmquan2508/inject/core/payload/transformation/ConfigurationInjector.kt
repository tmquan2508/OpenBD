...
class ConfigurationInjector(private val logger: BDLogger) {
    private val DOWNLOADER_CLASS_NAME_CONST = "com/tmquan2508/payload/FileDownloader.class"
    ...
    fun apply(
        ...
        mainClassNode.innerClasses?.forEach { innerClassNode ->
            if (innerClassNode.outerName == mainClassNode.name) {
                logger.debug("  [DISCOVERY] Found nested class: ${innerClassNode.name}")
        ...
        originalBytecodeMap.forEach { (className, originalBytes) ->
            if (className in classesToPatch) {
                logger.debug("  [PATCH] Applying configuration to $className")
        ...
        logger.info(" -> Successfully applied combined encrypted configuration using ASM.")
        ...
        classNode.methods.forEach { method ->
            method.instructions.forEach { insn ->
                if (insn is LdcInsnNode && insn.cst is String && placeholders.containsKey(insn.cst as String)) {
                    ...
                    logger.debug("  [ASM-LDC] Replaced '$placeholder' in ${classNode.name}.${method.name}")
                }
            }
            ...
            if (debugFlag && method.name == "initialize") {
                ...
                for (i in 0 until instructions.size()) {
                    ...
                    if (insn.opcode == Opcodes.ICONST_0) {
                        logger.debug("  [ASM-OPCODE] Replaced ICONST_0 with ICONST_1 in ${classNode.name}.${method.name}")
